...
const sqlFF = document.querySelector('.sqlB').innerHTML;
const sqlFFF = sqlFF.replace('`', "'");
const sqlFFFF = sqlFFF.replace('`', "'");
const sqlFFFF2 = sqlFFFF.replace('*', "count(*)");
...
function getPagiInfo() {
    ...
    // Настройка запроса
    request.open('GET','/getPagiSql/?query='+sqlFFFF2,true);
...
window.onload = function () {
    ...
    //let sql1 = 'SELECT * FROM "modScheme"."mod" ORDER BY "date_time" DESC';
    getObjects(sqlFFFF, 7, 0);
